...
export function getEnvFilePath(config: Config, env: EnvType): string {
  const files = config.envFiles ?? { dev: '.env.development', prod: '.env.production' }
...
export async function loadEnvFile(
  ...
  if (await keysFile.exists()) {
    ...
    // 根据文件名提取对应的私钥
    const envName = envPath.includes('production') ? 'PRODUCTION' : 'DEVELOPMENT'
    const keyMatch = keysContent.match(new RegExp(`DOTENV_PRIVATE_KEY_${envName}=["']?([^"'\n]+)["']?`))
    if (keyMatch) {
      privateKey = keyMatch[1]
    } else {
      // 尝试通用私钥
      const defaultMatch = keysContent.match(/DOTENV_PRIVATE_KEY=["']?([^"'\n]+)["']?/)
      if (defaultMatch) {
...
export function shouldExclude(key: string, excludePatterns: string[]): boolean {
  const allPatterns = [...BUILTIN_EXCLUDE_PREFIXES.map(p => p + '*'), ...excludePatterns]
